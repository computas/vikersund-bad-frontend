...
export function GruppeAktivitetForm({
  ...
  const [aktivitetType, setAktivitetType] = useState(
    avtale?.aktivitetType ?? "",
  );
  const [behandlerId, setBehandlerId] = useState<number | null>(
    avtale?.behandlerId ?? null,
  );
  ...
  function validate(): string | null {
    ...
    const conflicting = gruppeAktiviteter
      .filter((p) =>
        avtale
          ? !(p.dag === originalDag && p.startTid === avtale.startTid)
          : true,
      )
      .filter((p) => p.dag === dag)
    ...
    if (behandlerId !== null) {
      ...
      if (behandlerConflict) {
        ...
        const gruppeInfo = behandlerConflict.gruppeNavn
          ? ` (${behandlerConflict.gruppeNavn})`
          : "";
  ...
  async function handleSave() {
    ...
    try {
      await onSave({
        ...
        startTid,
        sluttTid,
        aktivitetNavn: aktivitetNavn.trim(),
        aktivitetType: aktivitetType || null,
        behandlerId,
      });
      setSaved(true);
      setTimeout(() => {
    ...
    <>
      ...
      <div className="grid gap-4 text-base">
        ...
        <div className="flex flex-col gap-1">
          ...
          <select
            value={aktivitetType}
            onChange={(e) => setAktivitetType(e.target.value)}
            className={SELECT_CLASS}
          >
        ...
        <div className="flex flex-col gap-1">
          <label className={LABEL_CLASS}>
            Behandler <span className="normal-case">(valgfri)</span>
          </label>
          <select
            value={behandlerId ?? ""}
            onChange={(e) =>
        ...
        {onDelete && !isCreateMode && (
          confirmDelete ? (
            <div className="flex items-center gap-2">
              <span className="text-sm text-zinc-500 dark:text-zinc-400">Sikker?</span>
              <button
